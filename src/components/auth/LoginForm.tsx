...
export function LoginForm({
  redirectTo = '/admin',
  title = 'Logga in',
  description = 'Logga in med e-postlänk eller Google.',
  inviteHint,
  showCreateLink = true,
}: LoginFormProps) {
  ...
  const isBusy = isMagicLinkSubmitting || isGoogleSubmitting
    ...
    <div className="relaxed-surface mx-auto flex w-full max-w-xl flex-col gap-6 p-8">
      ...
      >
        ...
        <button
          type="submit"
          disabled={isBusy}
          className="relaxed-primary-button h-12 cursor-pointer px-6 text-base font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60"
        >
          {isMagicLinkSubmitting ? 'Skickar...' : 'Skicka inloggningslänk'}
        </button>
      </form>
